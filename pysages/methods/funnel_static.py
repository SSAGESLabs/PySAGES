...
class Static(GriddedSamplingMethod):
    """
    Funnel with Static.
    ...
    restart_file: str 
        .pkl file from previous pysages simulation. 
    ...
    def __init__(self, cvs, grid, restart_file, **kwargs):
        super().__init__(cvs, grid, **kwargs)
        run_result = pysages.load(restart_file)
        fe_result = pysages.analyze(run_result)
        self.force_array = np.asarray(-fe_result['mean_force'])
    ...
    def build(self, snapshot, helpers, *args, **kwargs):
        ...
        return _static(self, snapshot, helpers)


def _static(method, snapshot, helpers):
    """
    ...
    Returns
    -------
    Tuple `(snapshot, initialize, update)` to run static bias simulations.
    """
    cv = method.cv
    grid = method.grid
    force_array = method.force_array
    ...
    def update(state, data):
        ...
        force = estimate_force(xi, I_xi, force_array).reshape(dims)
...
@dispatch
def build_force_estimator(method: Static):
    """
    ...
    def bias_force(data):
        _, I_xi, force_array = data
        return force_array[I_xi]
    ...
    if method.restraints is None:
        ...
        def estimate_force(xi, I_xi, force_array):
            ob = np.any(np.array(I_xi) == grid.shape)  # Out of bounds condition
            data = (xi, I_xi, force_array)
