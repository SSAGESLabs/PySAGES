...
def post_run_action(**kwargs):
    hoomd.dump.gsd(filename=f"final_{kwargs.get('replica_num')}.gsd", overwrite=True, period=None, group=hoomd.group.all())
...
def main(argv):
    ...
    raw_result = pysages.run(method, generate_context, args.time_steps, post_run_action=post_run_action)
