...
# credits- [JAX MD NVT simulation tutorial](https://colab.research.google.com/github/google/jax-md/blob/main/notebooks/nvt_simulation.ipynb)
from utils import total_energy_fn, run_minimization, step_fn
...
def generate_simulation(pdb_filename=adp_pdb, T=T, dt=dt):
    def total_energy_fn(R, **kwargs):
        ...
        epsilon = epsilon.at[particles_index[0], particles_index[1]].set(0.0)
        epsilon = epsilon.at[particles_index[1], particles_index[0]].set(0.0)

        solvent_energy_fn = energy.lennard_jones_pair(
            displacement, sigma=1.0, epsilon=epsilon, r_cutoff=2.5
        )

        epsilon = 0 * epsilon
        particles_index = [0, 10]
        epsilon = epsilon.at[particles_index[0], particles_index[1]].set(5.0)
        epsilon = epsilon.at[particles_index[1], particles_index[0]].set(5.0)

        particle_energy_fn = energy.morse_pair(
            displacement, sigma=1.0, epsilon=epsilon, alpha=5.0, r_cutoff=2.5
        )

        return particle_energy_fn(R) + solvent_energy_fn(R)

    def run_minimization(energy_fn, R_init, shift, particles_index, epsilon, num_steps=5000):
        ...
        @jit
        def scan_fn(state, i):
            return apply(state, epsilon=epsilon, particles_index=particles_index), 0.0

        state = init(R_init, epsilon=epsilon, particles_index=particles_index)
        state, _ = lax.scan(scan_fn, state, np.arange(num_steps))

        return state.position, np.amax(
            np.abs(
                -grad(total_energy_fn)(
                    state.position, epsilon=epsilon, particles_index=particles_index
                )
            )
        )

    def step_fn(i, state_and_log, particles_index=particles_index):
        state, log, epsilon, particles_index = state_and_log
        ...
        H = simulate.nvt_nose_hoover_invariant(
            total_energy_fn, state, kT, epsilon=epsilon, particles_index=particles_index
        )
        ...
        state = apply(state, epsilon=epsilon, particles_index=particles_index)  # kT=kT)

        return state, log, epsilon, particles_index

    # RUN energy minimization
    Rfinal, max_force_component = run_minimization(
        total_energy_fn, R, shift, particles_index, epsilon
    )
...
def main(argv=[]):
    ...
    grid = pysages.Grid(lower=(0), upper=(2), shape=(50), periodic=True)
